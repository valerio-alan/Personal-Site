...
export default function Hero() {
	useEffect(() => {
		...
		heroChars.forEach((el) => {
			...
			el.addEventListener('mouseclick', (event) => {
				hoverChar(event)
			})
		...
		return () => {
			...
			heroChars.forEach((el) => {
				...
				el.removeEventListener('mouseclick', (event) => {
					hoverChar(event)
				})
	...
	heroText = heroText.map((char, i) => (
		<span key={i} className='hero-char' charnum={i}>
			{char}
		</span>
	))
	...
	return <Section header={false} id='hero' content={content} />
}
