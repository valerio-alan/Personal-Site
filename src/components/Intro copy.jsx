...
export default function Intro() {
    ...
    useEffect(() => {
        const nameText = document.getElementsByClassName('name-title')[0]
        const nameChars = [...document.getElementsByClassName('name-char')]

        function hoverChar(event) {
            let char = event.target
            let charNum = nameChars.indexOf(char)
            
            char.style = 'transition: bottom 100ms ease-out; bottom: 10px'
            char.setAttribute('chartype', 'primary')

            if (nameChars[charNum - 1]) {
                nameChars[charNum - 1].style = 'transition: bottom 100ms 25ms ease-out; bottom: 5px'
                nameChars[charNum - 1].setAttribute('chartype', 'secondary')
            }
        ...
        function leaveChar(event) {
            ...
            if (nameChars[charNum - 1]) {
                charList.push(nameChars[charNum - 1])
            }
        ...
        nameText.addEventListener('mouseleave', leaveWord)
        nameChars.forEach((el) => {
        ...
        return (() => {
            nameText.removeEventListener('mouseleave', leaveWord)
            nameChars.forEach((el) => {
    ...
    let nameText = [...'Alan Valerio']
    nameText = nameText.map((char, i) => (<span key={i} className="name-char" charnum={i}>{char}</span>))
    ...
    const content = (
        <>
            ...
            <div className='intro-text'>
                ...
                {desktopBubbles}
                <h3>Hello! My name is <span className='bold purple name-title'>{nameText}</span>.</h3>
